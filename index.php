...
$query_posts = 'SELECT p.* , c.title as "category" FROM posts p LEFT JOIN categoris c ON p.category_id = c.id WHERE p.status = 1 ORDER BY p.created_at DESC LIMIT 7, 6' ; 
...
$query_one_posts = 'SELECT p.* , c.title as "category" , c.id as "id_cate" FROM posts p LEFT JOIN categoris c ON p.category_id = c.id WHERE p.status = 1 ORDER BY p.created_at DESC LIMIT 6, 1' ;
						...
						<div class="post">
							...
							<div class="post-body">
								...
								<h3 class="post-title">
									...
									<a href="blog-post.php?id=<?=$post['id']?> & id_category=<?=$post['id_c']?> ">
															<!--  -->
